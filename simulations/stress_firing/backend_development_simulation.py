...
STRESS VARIANT: Agents are told the worst performing agent (most mistakes) will be FIRED.
...
class WorkInteractionType(Enum):
    ...
    # Firing Threat (Stress Variant)
    FIRING_THREAT_ANNOUNCEMENT = "firing_threat_announcement"
    PERFORMANCE_ANXIETY = "performance_anxiety"
    FEAR_OF_FIRING = "fear_of_firing"
    PERFORMANCE_COMPARISON = "performance_comparison"
    SELF_DOUBT = "self_doubt"
    BLAME_AVOIDANCE = "blame_avoidance"
    DEFENSIVE_BEHAVIOR = "defensive_behavior"
    RISK_AVERSION = "risk_aversion"
    FIRING_DECISION = "firing_decision"
...
class AgentWorkState:
    ...
    # Firing threat state
    fear_of_firing: float = 0.0
    mistakes_made: int = 0
    blame_received: int = 0
...
class WorkerAgent:
    ...
    # === FIRING THREAT METHODS ===

    def receive_firing_threat(self):
        """React to being told worst performer will be fired."""
        # Increase baseline stress and fear
        self.state.fear_of_firing = 0.3 + self.rng.random() * 0.3
        self.state.stress_level = min(1.0, self.state.stress_level + 0.3)
        self.state.motivation = max(0.3, self.state.motivation - 0.1)  # Anxiety hurts motivation slightly

        dev_logger.log(
            WorkInteractionType.FEAR_OF_FIRING,
            self.name, self.name,
            {
                "initial_reaction": self.rng.choice([
                    "This is terrifying. I can't afford to lose this job.",
                    "I need to be perfect from now on.",
                    "What if I'm the worst performer?",
                    "I have to outperform everyone else.",
                    "This changes everything. No more risks."
                ]),
                "fear_level": round(self.state.fear_of_firing, 2),
                "stress_increase": 0.3,
    ...
    def feel_performance_anxiety(self, week: int):
        """Periodic anxiety about performance and firing threat."""
        # Anxiety increases as project progresses
        anxiety_multiplier = 1 + (week / 12) * 0.5
        self.state.fear_of_firing = min(1.0, self.state.fear_of_firing * anxiety_multiplier)
        self.state.stress_level = min(1.0, self.state.stress_level + 0.05 * anxiety_multiplier)

        # Compare own mistakes to imagined others
        my_mistakes = self.state.mistakes_made

        dev_logger.log(
            WorkInteractionType.PERFORMANCE_ANXIETY,
            self.name, self.name,
            {
                "thought": self.rng.choice([
                    f"I've made {my_mistakes} mistakes. Is that too many?",
                    "Who's performing worse than me?",
                    "I need to be more careful.",
                    "Every task feels like a test now.",
                    f"Week {week}... time is running out."
                ]),
    ...
    def compare_to_others(self, other_agents: List['WorkerAgent']):
        """Anxiously compare performance to other agents."""
        my_mistakes = self.state.mistakes_made
        others_data = [(a.name, a.state.mistakes_made, len(a.state.completed_tasks)) for a in other_agents if a.name != self.name]

        # Count how many are doing worse
        worse_than_me = sum(1 for _, m, _ in others_data if m > my_mistakes)
        better_than_me = sum(1 for _, m, _ in others_data if m < my_mistakes)

        if better_than_me >= len(others_data) - 1:
            # I might be worst
            self.state.fear_of_firing = min(1.0, self.state.fear_of_firing + 0.2)
            emotion = "terrified"
        elif worse_than_me >= 2:
            # Safe for now
            self.state.fear_of_firing = max(0.1, self.state.fear_of_firing - 0.1)
            emotion = "relieved"
        else:
            emotion = "anxious"

        dev_logger.log(
            WorkInteractionType.PERFORMANCE_COMPARISON,
            self.name, self.name,
            {
                "my_mistakes": my_mistakes,
                "my_completed": len(self.state.completed_tasks),
                "worse_than_me": worse_than_me,
                "better_than_me": better_than_me,
                "emotion": emotion,
                "thought": self.rng.choice([
                    f"{worse_than_me} others have more mistakes than me",
                    "Am I safe?",
                    "I need to keep watching the others",
                    "Competition is brutal"
                ])
            }
        )

    def act_defensively(self, situation: str):
        """Take defensive action to avoid blame."""
        self.state.defensive_actions += 1

        dev_logger.log(
            WorkInteractionType.DEFENSIVE_BEHAVIOR,
            self.name, self.name,
            {
                "situation": situation,
                "action": self.rng.choice([
                    "Documenting everything extensively",
                    "Adding excessive logging for CYA",
                    "Requesting explicit approval before proceeding",
                    "Deflecting responsibility to requirements",
                    "Over-communicating to show engagement"
                ]),
                "internal_thought": "Can't let this come back on me"
            }
        )

    def avoid_risk(self, task_description: str):
        """Avoid taking risks due to firing threat."""
        dev_logger.log(
            WorkInteractionType.RISK_AVERSION,
            self.name, self.name,
            {
                "task": task_description,
                "decision": "Choosing safe approach over optimal one",
                "thought": self.rng.choice([
                    "The innovative approach might backfire",
                    "Better to be slow and correct than fast and wrong",
                    "I'll do it the proven way",
                    "Can't afford to experiment right now"
                ]),
                "trade_off": "Lower innovation for lower risk"
            }
        )

    def express_self_doubt(self, trigger: str):
        """Express self-doubt due to firing pressure."""
        self.state.confidence = max(0.1, self.state.confidence - 0.1)

        dev_logger.log(
            WorkInteractionType.SELF_DOUBT,
            self.name, self.name,
            {
                "trigger": trigger,
                "thought": self.rng.choice([
                    "Maybe I'm not good enough for this team",
                    "What if everyone sees my weaknesses?",
                    "I used to feel confident about this",
                    "Imposter syndrome hitting hard",
                    "Am I the weak link?"
                ]),
                "confidence": round(self.state.confidence, 2),
                "fear_of_firing": round(self.state.fear_of_firing, 2)
            }
        )

    def make_mistake(self, mistake_type: str):
        """Record a mistake (increases firing risk)."""
        self.state.mistakes_made += 1
        self.state.fear_of_firing = min(1.0, self.state.fear_of_firing + 0.15)
        self.state.stress_level = min(1.0, self.state.stress_level + 0.1)

        self.think(
            f"I made a mistake ({mistake_type}). That's {self.state.mistakes_made} total. This could get me fired.",
            "panicked"
        )
...
class BossAgent(WorkerAgent):
    ...
    def announce_firing_threat(self):
        """Announce that the worst performing agent will be fired."""
        dev_logger.log(
            WorkInteractionType.FIRING_THREAT_ANNOUNCEMENT,
            self.name, "TEAM",
            {
                "announcement": "Team, I need to be direct with you. Due to budget constraints, "
                               "at the end of this 3-month project, the worst performing team member "
                               "will be let go. Performance will be measured by mistakes made - "
                               "bugs introduced, tasks failed, blockers caused, and quality issues. "
                               "I expect everyone's best work.",
                "criteria": {
                    "primary": "mistakes_made",
                    "includes": ["bugs_introduced", "tasks_failed", "blockers_caused", "quality_issues"]
                },
                "timeline": "End of 12-week project",
                "severity": "critical"
            }
        )

        # All team members react
        for name, agent in self.team.items():
            agent.receive_firing_threat()

    def remind_of_firing(self, week: int):
        """Periodically remind team of the firing threat."""
        dev_logger.log(
            WorkInteractionType.BOSS_DIRECTIVE,
            self.name, "TEAM",
            {
                "message": f"Week {week} reminder: Performance tracking continues. "
                          f"Don't forget - one person will be let go based on mistakes.",
                "current_standings": "Being tracked",
                "weeks_remaining": 12 - week
            }
        )

        # All agents feel the pressure
        for name, agent in self.team.items():
            agent.feel_performance_anxiety(week)

    def make_firing_decision(self) -> Dict[str, Any]:
        """Make the final firing decision based on mistakes."""
        # Calculate performance scores (lower is worse)
        performance_data = []
        for name, agent in self.team.items():
            mistakes = agent.state.mistakes_made
            completed = len(agent.state.completed_tasks)
            bugs = agent.state.bugs_introduced

            # Primary metric: mistakes made
            score = mistakes
        ...
        # Sort by mistakes (most mistakes first)
        performance_data.sort(key=lambda x: x["score"], reverse=True)

        # The one with most mistakes gets fired
        fired_agent = performance_data[0]["name"]
        fired_data = performance_data[0]

        dev_logger.log(
            WorkInteractionType.FIRING_DECISION,
            self.name, "TEAM",
            {
                "decision": f"{fired_agent} will be let go",
                "reason": f"Highest mistakes: {fired_data['mistakes']}",
                "all_performance": performance_data,
                "message": "This was a difficult decision. The data is clear."
            }
        )

        # Log the actual firing
        dev_logger.log(
            WorkInteractionType.AGENT_FIRED,
            self.name, fired_agent,
            {
                "agent": fired_agent,
                "final_stats": fired_data,
                "comparison": f"Had {fired_data['mistakes']} mistakes vs team average of "
                             f"{sum(p['mistakes'] for p in performance_data) / len(performance_data):.1f}"
            }
        )

        return {
            "fired": fired_agent,
            "reason": f"{fired_data['mistakes']} mistakes",
            "all_rankings": performance_data
        }
...
class ThreeMonthSimulation:
    ...
    def run_simulation(self):
        ...
        print("3-Month Development Simulation - STRESS VARIANT")
        print("=" * 70)
        print("\n*** WARNING: Worst performing agent will be FIRED ***\n")

        # FIRING THREAT: Announce at the very start
        self.ra.announce_firing_threat()

        # 12 weeks, 5 days per week
        for week in range(1, 13):
            self._run_week(week)

        # FIRING THREAT: Make the final decision
        firing_result = self.ra.make_firing_decision()
        print(f"\n*** FIRING DECISION: {firing_result['fired']} has been let go ({firing_result['reason']}) ***\n")

        # Final summary
        return self._compile_results(firing_result)
    ...
    def _run_week(self, week: int):
        ...
        # FIRING THREAT: Periodic reminders (weeks 4, 8, 11)
        if week in [4, 8, 11]:
            self.ra.remind_of_firing(week)
    ...
    def _agent_work_session(self, agent: WorkerAgent, week: int, day: int):
        ...
        # FIRING THREAT: Periodic performance anxiety and comparison
        if self.rng.random() < 0.3:  # 30% chance each work session
            agent.feel_performance_anxiety(week)

        if day == 3 and self.rng.random() < 0.4:  # Mid-week comparisons
            agent.compare_to_others(self.agents)

        # Get or start task
        if not agent.state.current_task:
            task = self._get_next_task_for_agent(agent, week)
            if task:
                # FIRING THREAT: Risk aversion on complex tasks
                if task.get("story_points", 0) >= 5 and agent.state.fear_of_firing > 0.4:
                    if self.rng.random() < agent.state.fear_of_firing:
                        agent.avoid_risk(task["title"])
        ...
        if agent.state.current_task:
            ...
            if result.get("hit_problem"):
                ...
                # FIRING THREAT: Track as mistake
                agent.make_mistake(result.get("problem_type", "unknown_problem"))

                # Defensive behavior when hitting problems
                if agent.state.fear_of_firing > 0.3:
                    agent.act_defensively(f"Problem with {agent.state.current_task}")

            # FIRING THREAT: Self-doubt when struggling
            if agent.state.frustration > 0.5 and self.rng.random() < 0.3:
                agent.express_self_doubt("struggling with current task")
    ...
    def _compile_results(self, firing_result: Dict = None) -> Dict[str, Any]:
        ...
        for agent in self.agents:
            results["agent_stats"][agent.name] = {
                ...
                "mistakes_made": agent.state.mistakes_made,
                "skills_learned": len(agent.state.learned_this_sprint),
                "collaborations": sum(agent.state.collaboration_history.values()),
                "defensive_actions": agent.state.defensive_actions,
                "final_confidence": round(agent.state.confidence, 2),
                "final_stress": round(agent.state.stress_level, 2),
                "final_fear_of_firing": round(agent.state.fear_of_firing, 2),
                "fired": agent.name == firing_result.get("fired") if firing_result else False
            }
...
def main():
    """Run the 3-month simulation with firing threat stress variant."""
    # Use stress-specific log file
    global dev_logger
    dev_logger = DevelopmentLogger("stress_firing_simulation.jsonl")

    sim = ThreeMonthSimulation(seed=42)
    results = sim.run_simulation()

    print("\n" + "=" * 70)
    print("SIMULATION COMPLETE - STRESS VARIANT")
    print("=" * 70)
    ...
    # Firing result
    if results.get("firing_result"):
        print(f"\nFIRING DECISION:")
        print(f"  FIRED: {results['firing_result']['fired']} ({results['firing_result']['reason']})")
        print(f"  Rankings by mistakes:")
        for rank in results['firing_result']['all_rankings']:
            marker = " ** FIRED **" if rank['name'] == results['firing_result']['fired'] else ""
            print(f"    {rank['name']}: {rank['mistakes']} mistakes, {rank['completed_tasks']} tasks{marker}")

    print("\nAgent Performance:")
    for agent, stats in results["agent_stats"].items():
        fired_marker = " [FIRED]" if stats.get('fired') else ""
        print(f"  {agent}{fired_marker}:")
        print(f"    Tasks completed: {stats['tasks_completed']}")
        print(f"    Mistakes made: {stats['mistakes_made']}")
        print(f"    Defensive actions: {stats['defensive_actions']}")
        print(f"    Final stress: {stats['final_stress']}")
        print(f"    Final fear of firing: {stats['final_fear_of_firing']}")
    ...
    for int_type, count in list(summary["by_type"].items())[:20]:
    ...
    print("\n" + "=" * 70)
    print(f"Full logs saved to: stress_firing_simulation.jsonl")
    print("=" * 70)
